        ...
        for texto in textos_referencia:
            if texto in texto_pagina:
                ...
                if texto == textos_referencia[0]:  # Se for o primeiro texto
                    ...
                    parcelamentos_encontrados = re.findall(r'Parcelamento\s+(.*)', texto_pagina)
                    for index, parcelamento in enumerate(parcelamentos_encontrados):
                        # Cria variáveis dinamicamente (pendencia1, pendencia2, etc.)
                        vars()[f'pendencia{index + 1}'] = parcelamento
